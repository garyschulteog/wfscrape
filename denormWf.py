...
ES_URL = "http://localhost:9200/recon/_doc/%s"
cookies = {
    '_delphi_session': 'ADD_A_REAL_DELPHIUS_KEY_HERE_OR_PARAMETERIZE_THIS_SCRIPT',
}

headers = {
    'content-type': 'application/json',
    'cookie': '_delphi_session=ADD_A_REAL_DELPHIUS_KEY_HERE_OR_PARAMETERIZE_THIS_SCRIPT',
}
	...
	for entity_id in wf_entities:
		...
		for wf in wfs:
			...
			if (resp.status_code >= 500):
				...
				#update workforce with cost elements
				for ce in resp.json()['data']:
					ceId = ce['_id']
					wf['configuration']['costElementsInfo'][ceId] = ce
			...
			for pgrpid in posReqGrps:
				...
				if (resp.status_code >=400 and resp.status_code < 500):
					errors.append("entity: `%s` wf: `%s` pgrp: `%s` message: `%s`" % (entity_id, wfid, pgrpid, resp.json()['message']))
				elif (resp.status_code >= 500): 
					errors.append("entity: `%s` wf: `%s` pgrp: `%s` code: %d" % (entity_id, wfid, pgrpid, resp.status_code))
				else:
